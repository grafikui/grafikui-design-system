...
export default function TypographyPage() {
  ...
  return (
    <motion.div initial="hidden" animate="show" variants={container} className="max-w-6xl flex flex-col gap-24 lg:gap-32 font-sans pt-12 md:pt-0">
      ...
      {/* Families — val is a plain string in the flattened JSON */}
      <motion.div variants={item} className="flex flex-col gap-12 lg:gap-16">
        ...
        <div className="flex flex-col gap-8 lg:gap-12">
          {Object.entries(typo.family).map(([key, val]: any) => (
            <motion.div
            ...
            >
              ...
              {/* val is the resolved font stack string */}
              <span className="font-mono text-[10px] text-white/30 uppercase tracking-[0.25em] relative z-10">{val}</span>
            </motion.div>
          ))}
        </div>
      </motion.div>

      {/* Sizes — val is a plain string e.g. "16px" */}
      <motion.div variants={item} className="flex flex-col gap-12 lg:gap-16">
        ...
        <div className="flex flex-col">
          {Object.entries(typo.size).map(([key, val]: any) => (
            <motion.div
